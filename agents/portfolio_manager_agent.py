...
def give_stock_recommendation(final_results: dict) -> str:
    """
    Given stock technical analysis data, sentiment scores, and stock prediction data, give a concise buy/sell/hold recommendation with reasoning.
    Give explanation based on each of the analyses.
    """
    try: 
        # Invoke the llm to analyze the final results and give a recommendation
        prompt = f"""You are a financial expert. Based on the following data for various companies, provide a concise recommendation (buy/sell/hold) for each company along with a brief reasoning."""
